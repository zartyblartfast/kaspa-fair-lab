...
## env-025 local SubmitTransactionRequest artifact

```text
Run ID: env-025
Date/time: 2026-06-23T20:24:30Z
Network: TN12/testnet scope only (local no-broadcast only; no live network access attempted)
...
- `kaspa_rpc_core::SubmitTransactionRequest`
- existing local `RpcTransaction` from `let rpc_tx: RpcTransaction = (&tx).into();`
...
Whether a local SubmitTransactionRequest object was constructed:
- Yes.
- Local code constructs `SubmitTransactionRequest { transaction: rpc_tx.clone(), allow_orphan: false }`.
...
```text
Run ID: env-044
Date/time: 2026-06-24T13:37:57Z
Network: TN12/testnet-12
...
Hardening result:
- hardened directory/manifest guard passed: yes
- evidence: the run launched `kaspad` via `cargo run --release --manifest-path /root/.cargo/git/checkouts/rusty-kaspa-410e06d1fde91a92/42b734f/Cargo.toml --bin kaspad ...`
- evidence: the earlier repo-root Cargo failure (`could not find Cargo.toml in /root/kaspa-fair-lab or any parent directory`) did not recur
...
Observation duration:
- readiness confirmed at 2026-06-24T13:09:21Z
- `kaspad exited during observation` logged at 2026-06-24T13:37:57Z
- observed runtime after readiness: 1716 seconds (28m36s)
- interpretation: approximately 30 minutes, but short of a full uninterrupted 30-minute window
...
- prior end-state read-only artifacts still present but not regenerated by env-044:
...
Read-only RPC results:
- getServerInfo: not freshly captured in env-044
- getBlockDagInfo: not freshly captured in env-044
- getSyncStatus: not freshly captured in env-044
...
Sync-progress comparison against prior zero/false baseline:
- prior baseline: blockCount=0, headerCount=0, virtualDaaScore=0, isSynced=false
- sync progress occurred in logs: true
- evidence examples from updated runner log:
  - `IBD: Processed 363640 block headers (33%)`
  - `IBD: Processed 417842 block headers (38%)`
- final fresh RPC-visible improvement captured in env-044: no, because final read-only suite did not run

Teardown result:
- node startup succeeded: true
- node stopped cleanly: partially; the process exited before the planned endpoint, but no approved listeners remained afterward
- post-run `ss -ltnp` verification showed no listeners on 127.0.0.1:16210, 127.0.0.1:16311, or 127.0.0.1:17210
- listeners were cleared: true
...
Notes:
- This rerun proves the env-043 hardening fixed the caller-cwd/Cargo-manifest problem.
- This rerun does not provide fresh end-of-run read-only RPC artifacts because the node exited before the script reached its final capture phase.
- The strongest next action remains another explicitly approved localhost-only readiness/sync characterization step that prioritizes obtaining fresh end-state read-only artifacts after a stable observation window.
```
...
```text
Run ID: env-037
Date/time: 2026-06-23T22:59:41Z
Network: TN12/testnet-12

Files changed:
- spikes/tn12-minimal-covenant/artifacts/env-037-kaspad-startup.log
- spikes/tn12-minimal-covenant/artifacts/env-037-get-server-info.txt
- spikes/tn12-minimal-covenant/findings.md
- spikes/tn12-minimal-covenant/README.md
...
- getServerInfo artifact path: `spikes/tn12-minimal-covenant/artifacts/env-037-get-server-info.txt`
...
Pass/fail result:
- node_startup_succeeded=true
- get_server_info_succeeded=true
- rpc_call_count=1
...
- whether anything was signed: false
- whether anything was submitted/broadcast: false
- stop condition reached: stopped immediately after the single read-only `getServerInfo` result was captured
